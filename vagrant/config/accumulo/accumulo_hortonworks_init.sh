...
start() {
    ...
    if [ $(su -s /bin/bash $RUN_AS -c "/usr/bin/hadoop fs -ls /user | grep accumulo | wc -l") == "0" ]; then
        echo "Creating accumulo user in hdfs"
        su -s /bin/bash $RUN_AS -c "/usr/bin/hadoop fs -mkdir -p /user/accumulo"
        su -s /bin/bash $RUN_AS -c "/usr/bin/hadoop fs -chown accumulo /user/accumulo"
    fi
